...
stock_charger_stack <- data.frame(width = c(18, 16, 14, 8), 
                                  thickness = c(0.1, 0.1, 0.1, 0.4))
# run restackor.exe
run_shimstack(shim_df = stock_charger_stack,
              outfile = file.path(results_loc, 'stocker_charger.csv'))

#plot outcome
result <- tidy_restackor_results(file.path(results_loc, 'stocker_charger.csv'))
